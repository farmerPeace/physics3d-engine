...
Quaternion::Quaternion(real w, real x, real y, real z): w_(w), x_(x), y_(y), z_(z) {}
Quaternion::Quaternion(real w, const Vector3D &vect): w_(w), x_(vect.x()), y_(vect.y()), z_(vect.z()) {}
...
Quaternion Quaternion::FromAxisAngle(const Vector3D &axis, real angle) {
    Vector3D norm_axis = axis.Normalized();
    const real half = angle * 0.5f;
    const real sin = std::sin(half);
    const real cos = std::cos(half);
...
Quaternion Quaternion::FromEulerAngles(real pitch, real yaw, real roll) {
    ...
    //   y =  cr·sy·cp + sr·cy·sp
    //   z =  sr·cy·cp − cr·sy·sp
    const real half_pitch = pitch * 0.5f;
    const real half_yaw = yaw   * 0.5f;
    const real half_roll = roll  * 0.5f;
 
    const real cos_pitch = std::cos(half_pitch), sin_pitch = std::sin(half_pitch);
    const real cos_yaw = std::cos(half_yaw), sin_yaw = std::sin(half_yaw);
    const real cos_roll = std::cos(half_roll), sin_roll = std::sin(half_roll);
...
Quaternion Quaternion::FromRotationMatrix(const Matrix3D &m) {
    ...
    const real trace = m(0,0) + m(1,1) + m(2,2);
 
    if (trace > 0.0f) {
        const real s = 0.5f / std::sqrt(trace + 1.0f);
        return Quaternion(
            0.25f / s,
            (m(2,1) - m(1,2)) * s,
            (m(0,2) - m(2,0)) * s,
        ...
        const real s = 2.0f * std::sqrt(1.0f + m(0,0) - m(1,1) - m(2,2));
        return Quaternion(
            (m(2,1) - m(1,2)) / s,
            0.25f * s,
            (m(0,1) + m(1,0)) / s,
        ...
        const real s = 2.0f * std::sqrt(1.0f + m(1,1) - m(0,0) - m(2,2));
        return Quaternion(
            (m(0,2) - m(2,0)) / s,
            (m(0,1) + m(1,0)) / s,
            0.25f * s,
        ...
        const real s = 2.0f * std::sqrt(1.0f + m(2,2) - m(0,0) - m(1,1));
        return Quaternion(
            (m(1,0) - m(0,1)) / s,
            (m(0,2) + m(2,0)) / s,
            (m(1,2) + m(2,1)) / s,
...
real Quaternion::operator [] (int index) const {
...
Quaternion &Quaternion::operator *= (const real &scalar) {
    w_ *= scalar;
    x_ *= scalar;
    y_ *= scalar;
    z_ *= scalar;
...
Quaternion &Quaternion::operator *= (const Quaternion &rhs) {
    const real w = w_ * rhs.w_ - x_ * rhs.x_ - y_ * rhs.y_ - z_ * rhs.z_;
    const real x = w_ * rhs.x_ + x_ * rhs.w_ + y_ * rhs.z_ - z_ * rhs.y_;
    const real y = w_ * rhs.y_ - x_ * rhs.z_ + y_ * rhs.w_ + z_ * rhs.x_;
    const real z = w_ * rhs.z_ + x_ * rhs.y_ - y_ * rhs.x_ + z_ * rhs.w_;
...
Quaternion Quaternion::Slerp (const Quaternion &a, const Quaternion &b, real t) {
    ...
    if (dot > 1.0f - PHYS_EPSILON) {
        return Nlerp(a, qb, t);
    }
 
    const real theta_0  = std::acos(dot);
    const real theta    = theta_0 * t;
    const real sin_t    = std::sin(theta);
    const real sin_t0   = std::sin(theta_0);
 
    const real s0 = std::cos(theta) - dot * sin_t / sin_t0;
    const real s1 = sin_t / sin_t0;
 
    return (a * s0 + qb * s1).Normalized();
}
 
Quaternion Quaternion::Nlerp (const Quaternion &a, const Quaternion &b, real t) {
    t = std::max(0.0f, std::min(1.0f, t));
...
Matrix3D Quaternion::ToRotationMatrix () const {
    ...
    const real w = n.w_, x = n.x_, y = n.y_, z = n.z_;
...
Vector3D Quaternion::ToEulerAngles () const {
    ...
    const real sin_pitch = R(2, 1);
    const real cos_pitch = R(2, 2);
    const real pitch = std::atan2(sin_pitch, cos_pitch);
    
    // Yaw: extraer de R[0,0], R[1,0] y R[2,0]
    const real sin_yaw = -R(2, 0);
    const real cos_yaw = std::sqrt(R(0, 0) * R(0, 0) + R(1, 0) * R(1, 0));
    const real yaw = std::atan2(sin_yaw, cos_yaw);
    
    // Roll: extraer de R[1,0] y R[0,0]
    const real sin_roll = R(1, 0) / (cos_yaw + PHYS_EPSILON);
    const real cos_roll = R(0, 0) / (cos_yaw + PHYS_EPSILON);
    const real roll = std::atan2(sin_roll, cos_roll);
...
bool Quaternion::operator == (const Quaternion &rhs) const {
    return (std::abs(w_ - rhs.w_) <= PHYS_EPSILON &&
            std::abs(x_ - rhs.x_) <= PHYS_EPSILON &&
...
Quaternion Quaternion::Exponential () const {
    ...
    const real v_mag = v.Magnitude();
    const real e_w   = std::exp(w_);
 
    if (v_mag < PHYS_EPSILON) {
        return Quaternion(e_w, 0.0f, 0.0f, 0.0f);
    }
 
    const real sin_v = std::sin(v_mag);
    const real scale = e_w * sin_v / v_mag;
...
Quaternion Quaternion::Logarithm () const {
    ...
    const Vector3D v(x_, y_, z_);
    const real v_mag = v.Magnitude();
    const real log_w = std::log(mag);
 
    if (v_mag < PHYS_EPSILON) {
        return Quaternion(log_w, 0.0f, 0.0f, 0.0f);
    }
 
    const real angle = std::acos(std::max(-1.0f, std::min(1.0f, w_ / mag)));
    const real scale = angle / v_mag;
...
const real *Quaternion::Data () const {
    return &w_;
}
 
real *Quaternion::Data () {
    return &w_;
}