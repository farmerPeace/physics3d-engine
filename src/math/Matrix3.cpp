...
Matrix3::Matrix3() {
    matrix[0] = 0.0f; matrix[1] = 0.0f; matrix[2] = 0.0f;
    matrix[3] = 0.0f; matrix[4] = 0.0f; matrix[5] = 0.0f;
    matrix[6] = 0.0f; matrix[7] = 0.0f; matrix[8] = 0.0f;
}
...
Matrix3 Matrix3::Rotation(float angle, const Vector3D &axis) {
    ...
    return Matrix3(
        t * x * x + c, t * x * y + s * z, t * x * z - s * y,  // Columna 0
        t * x * y - s * z, t * y * y + c, t * y * z + s * x,  // Columna 1  
        t * x * z + s * y, t * y * z - s * x, t * z * z + c   // Columna 2
    );
}
...
Matrix3 Matrix3::operator * (const Matrix3 &rhs) const {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            float sum = 0;
            for (int k = 0; k < 3; k++) {
                sum += matrix[i * 3 + k] * rhs.matrix[k * 3 + j];
            }
...
Matrix3 &Matrix3::operator *= (const Matrix3 &rhs) {
    ...
    // Calcular multiplicación directamente
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            float sum = 0;
            for (int k = 0; k < 3; k++) {
                sum += temp[i * 3 + k] * rhs.matrix[k * 3 + j];
            }
            matrix[i * 3 + j] = sum;
        }
    }
...
Matrix3 Matrix3::Inverted() const {
    ...
    if (det == 0) {
        // Devolver matriz identidad o lanzar excepción
    ...
    result.matrix[1] = (matrix[2]*matrix[7] - matrix[1]*matrix[8]) * invDet;
    result.matrix[2] = (matrix[1]*matrix[5] - matrix[2]*matrix[4]) * invDet;
    
    result.matrix[3] = (matrix[5]*matrix[6] - matrix[3]*matrix[8]) * invDet;
    result.matrix[4] = (matrix[0]*matrix[8] - matrix[2]*matrix[6]) * invDet;
    result.matrix[5] = (matrix[2]*matrix[3] - matrix[0]*matrix[5]) * invDet;
    
    result.matrix[6] = (matrix[3]*matrix[7] - matrix[4]*matrix[6]) * invDet;
    result.matrix[7] = (matrix[1]*matrix[6] - matrix[0]*matrix[7]) * invDet;
...
Matrix3& Matrix3::Invert() {
    ...
    matrix[1] = (m02*m21 - m01*m22) * invDet;
    matrix[2] = (m01*m12 - m02*m11) * invDet;
    
    matrix[3] = (m12*m20 - m10*m22) * invDet;
    matrix[4] = (m00*m22 - m02*m20) * invDet;
    matrix[5] = (m02*m10 - m00*m12) * invDet;
    
    matrix[6] = (m10*m21 - m11*m20) * invDet;
    matrix[7] = (m01*m20 - m00*m21) * invDet;
